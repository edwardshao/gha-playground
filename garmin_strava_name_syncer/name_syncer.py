...
def init_garmin_client():
    """Initialize Garmin API via garth with base64-encoded tokens."""
    try:
        print(
            "ℹ️ Trying to login to Garmin Connect using token data from GARMIN_TOKENS_BASE64 env..."
        )

        garth.client.loads(GARMIN_TOKENS_BASE64)

    except (FileNotFoundError, GarthHTTPError) as err:
...
def get_garmin_activities(after_datetime_utc, before_datetime_utc):
    ...
    success, activities, error_msg = garmin_safe_api_call(
        Activity.list_by_date, after_date, before_date
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        success, garmin_activities = get_garmin_activities(
            after_datetime_utc, before_datetime_utc
        )
